...
scene::SceneAstronomy::SceneAstronomy() {
    engine::Debug::enable_gl_debugging();

    shader_ = std::make_unique<engine::Shader>("assets/shaders/basic.shader");
    shader_->bind();

    texture_ = std::make_unique<engine::Texture>("assets/textures/ball.png");
...
void scene::SceneAstronomy::update(float delta_time) {
    camera_controller_->update(delta_time);
}

void scene::SceneAstronomy::render() {
...
void scene::SceneAstronomy::imgui_render() {
